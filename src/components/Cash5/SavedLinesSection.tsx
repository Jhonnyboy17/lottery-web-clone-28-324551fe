...
const SavedLinesSection: React.FC<SavedLinesSectionProps> = ({
  ...
  return (
    <div className="bg-gray-50 dark:bg-gray-800 p-4">
      <h3 className="font-semibold mb-3 dark:text-gray-200">Minhas Linhas</h3>
      
      {savedLines.length === 0 ? (
        // Display a template line with question marks
        <div className="mb-2">
          <div 
            className="bg-white dark:bg-gray-700 rounded p-3 flex items-center justify-between cursor-pointer hover:bg-gray-50 dark:hover:bg-gray-600"
          ...
          >
            <div className="flex items-center">
              ...
              {Array(3).fill(null).map((_, i) => (
                <span 
                  key={i} 
                  className="bg-white dark:bg-gray-600 border border-gray-200 dark:border-gray-500 text-gray-700 dark:text-gray-300 font-bold rounded-full w-10 h-10 flex items-center justify-center text-sm mx-0.5"
                >
                  ?
                </span>
              ))}
            ...
            <div key={index} className="mb-2">
              <div 
                className={`rounded p-3 flex items-center justify-between cursor-pointer transition-colors ${
                  editingIndex === index ? 'bg-blue-100 dark:bg-blue-900/30' : 'bg-white dark:bg-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600'
                }`}
                  ...
                  {line.digits.map((digit, i) => (
                    <span 
                      ...
                      className={`rounded-full w-10 h-10 flex items-center justify-center text-sm mx-0.5 ${
                        digit === null ? 'bg-white dark:bg-gray-600 border border-gray-200 dark:border-gray-500 text-gray-700 dark:text-gray-300 font-bold' : 
      ...
      {editingIndex === null && (
        <div className="mb-2">
          <div 
            className="bg-white dark:bg-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600 rounded p-3 flex items-center justify-between cursor-pointer"
            onClick={onStartNewLine}
          >
            <div className="flex items-center">
              <span className="text-gray-500 dark:text-gray-400 font-medium w-6 mr-2">
